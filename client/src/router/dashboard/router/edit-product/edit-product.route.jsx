...
const EditProduct = () => {
  ...
  const handleArchive = async () => {
    try {
      ...
      } else window.alert("Product not archived");
    } catch (err) {
      console.log(err);
  ...
  const handlePublish = async () => {
    try {
      ...
      if (res.status === 200) {
        window.location.reload();
      } else window.alert("Product not published");
    } catch (err) {
      console.log(err);
      window.alert("Product not archived");
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      } else window.alert("Product not deleted");
    } catch (err) {
      console.log(err);
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 border rounded-md shadow-md">
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="submit"
              className="bg-gray-800 text-white py-2 px-4 rounded-md hover:bg-gray-700 h-12 col-span-2"
            >
              Edit Product
            </button>
            ...
            <span
              ...
              className="bg-gray-800 text-white py-2 px-4 rounded-md hover:bg-gray-700 h-12 flex justify-center items-center"
            >
            ...
            {productData.archived ? (
              <span
                ...
                className="bg-red-600 text-white py-2 px-4 rounded-md hover:bg-red-500 h-12 flex justify-center items-center col-span-2"
              >
                Delete
              </span>
            ) : null}
